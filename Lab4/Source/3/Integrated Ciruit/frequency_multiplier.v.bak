module datapath (input RefClk,clk,rst,cnt_en,cnt_ld,new_calc,input [2:0]n,output cout,output reg calc_k,out,output [15:0]k);

    reg prev_clk;
    reg [15:0] duration , counter , current;

    always @(posedge RefClk , posedge rst) begin
        if (rst) begin prev_clk = 0 ; out = 0 ; end
        else prev_clk <= clk;
    end
...
module controller (input clk,rst,adjust,calc_k,cout,output reg valid,cnt_en,cnt_ld,new_calc);
    
    reg [2:0]ps , ns;
    parameter [2:0] idle = 0 , ready = 1 , calculate = 2 , load_calculated = 3 , count = 4 , redo = 5 ;

	always @(posedge clk , adjust , calc_k , cout) begin
		case(ps)
...
module frequency_multiplier (input clk,rst,InFreq,adjust,input [2:0]n,output [15:0]k,output valid,outFreq);
    ...
    datapath dp(.RefClk(clk),.clk(InFreq),.rst(rst),.cnt_en(cnt_en),.cnt_ld(cnt_ld),.n(n),.cout(cout),.calc_k(calc_k),
                .out(outFreq),.k(k),.new_calc(new_calc));
    controller ct(.clk(clk),.rst(rst),.adjust(adjust),.calc_k(calc_k),.cout(cout),.valid(valid),.cnt_en(cnt_en),.cnt_ld(cnt_ld),
                  .new_calc(new_calc));
